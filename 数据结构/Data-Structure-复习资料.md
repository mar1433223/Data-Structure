...
> 基于项目 `Data-Structure`，按学习顺序从线性表 → 栈 → 队列 → 树 → 图 → 查找 → 排序的递进式复习文档。
...
6. [查找 (search)](#6-查找-search)
   - 6.1 二分查找 BinarySearch
   - 6.2 哈希查找 HashSearch
7. [排序 (sort)](#7-排序-sort)
   - 7.1 排序算法总览
   - 7.2 冒泡排序 BubbleSort
   - 7.3 简单选择排序 SelectionSort
   - 7.4 直接插入排序 InsertionSort
   - 7.5 希尔排序 ShellSort
   - 7.6 快速排序 QuickSort
   - 7.7 堆排序 HeapSort
   - 7.8 归并排序 MergeSort
   - 7.9 计数排序 CountingSort
...
当前 `search` 包的示例均通过 `Scanner` 读取标准输入，数组数据按下标 `1..n` 保存。`BinarySearch` 假定输入序列已升序；`HashSearch` 用长度为 `15` 的数组保存哈希表，空槽位以 `-1` 标记。

### 6.1 二分查找 `BinarySearch`

**核心操作**：在有序区间 `[i, j]` 中取中点 `mid = (i + j) / 2`。若 `a[mid] == k` 则找到；若 `a[mid] > k`，目标只能在左半区；否则只在右半区。当前实现输出找到的下标，找不到输出 `-1`。

每轮比较都会把候选区间近似减半，时间复杂度为 O(log n)，额外空间 O(1)。前提是数组必须有序；对无序数组直接二分没有意义。

**Java 实现要点与易错点**：

- 当前代码使用 1-based 下标，初始边界是 `i = 1`、`j = n`，循环条件必须为 `i <= j`，否则会漏掉最后一个候选位置。
- 一般库代码可写成 `mid = i + (j - i) / 2`，避免大下标相加溢出；当前练习数组较小，`(i + j) / 2` 不会溢出。
- 有重复值时，命中哪个下标取决于中点过程；若题目要求第一个或最后一个位置，需要在命中后继续向左或向右收缩边界。

**例题思路**：遇到“有序数组中查某值”“最小满足条件的下标”“答案范围单调”时，都可先确定判定条件，再把区间二分。后二类属于二分答案，循环中保存候选答案而不只是比较相等。

### 6.2 哈希查找 `HashSearch`

**当前实现**：`HashTable` 有容量 `m = 15` 的整型数组，`-1` 表示空位。`hash(k)` 使用除留余数法计算 `k % 13`；插入时若初始地址被占用，就按 `(i + d) % m` 线性探测下一个槽位。

平均情况下，哈希查找和插入可视为 O(1)，但性能取决于装填因子和冲突情况；本实现最坏要沿探测序列扫描整个表，为 O(m)。额外空间为 O(m)。

**Java 实现要点与易错点**：

- 当前空槽哨兵是 `-1`，所以不能把 `-1` 当作普通键值；`k % 13` 对负数也会得到负地址，因此练习输入应使用非负整数。
- 线性探测必须对容量 `m` 取模，否则探测到数组末尾会越界。
- 插入前应考虑满表。当前 `find` 在满表时会无限探测，示例应保证元素个数不超过容量且至少留有空槽。
- 查找应沿同一探测序列逐项比较键值，直到遇到空槽才可判定不存在。当前 `search` 方法的首槽命中路径只输出槽位，冲突后的路径用于展示探测过程；若作为通用查找函数，应在每个槽位比较 `a[newi] == p` 后立即返回，并在遇到 `-1` 时返回失败。

**例题思路**：题目给出表长、散列函数和关键字时，先算初始地址，再按指定冲突处理方法逐步写出探测位置。注意“查找长度”要计入第一次访问初始地址的比较。

---

## 7. 排序 (sort)

当前 `sort` 包中的示例都从标准输入读取 `n` 和 `n` 个整数，使用长度为 `105` 的数组，并按下标 `1..n` 保存数据。除希尔排序会输出每一趟结果外，其余实现最终输出升序序列。
...
| 希尔排序 | 与增量序列有关 | 与增量序列有关 | 当前折半增量通常记为 O(n²) | O(1) | 不稳定 |
| 快速排序 | O(n log n) | O(n log n) | O(n²) | 平均 O(log n) 递归栈 | 不稳定 |
| 堆排序 | O(n log n) | O(n log n) | O(n log n) | O(1) | 不稳定 |
| 归并排序 | O(n log n) | O(n log n) | O(n log n) | O(n) | 稳定 |
| 计数排序 | O(n + k) | O(n + k) | O(n + k) | O(n + k) | 稳定 |

判断稳定性时，要看相等元素排序后的相对顺序是否可能改变。当前实现都是原地排序，快速排序的递归栈空间不计入数组之外的显式辅助结构。

### 7.2 冒泡排序 `BubbleSort`

**核心操作**：从前向后比较相邻元素，如果 `a[j] > a[j + 1]` 就交换。完成第 `i` 趟后，当前未排序区间的最大值被放到右端。

当前源码用 `flag` 记录本趟是否发生交换；如果一趟没有交换，说明序列已经有序，可以提前结束。因此有序输入的时间复杂度是 O(n)，平均和最坏情况仍为 O(n²)。

**易错点**：

- 第 `i` 趟只需比较到 `n - i`，避免访问 `a[n + 1]`。
- 相等时不交换，才能保持稳定性。
- `flag` 必须在每一趟开始前重新置为 `false`。

**例题思路**：要求统计交换次数时，可在交换处累加；冒泡排序的交换次数等于序列的逆序对数量，但直接冒泡统计仍需 O(n²)。

### 7.3 简单选择排序 `SelectionSort`

**核心操作**：第 `i` 趟扫描区间 `i + 1..n`，记录最小元素下标 `min`，扫描完成后执行一次 `swap(a, min, i)`。

无论输入是否有序，都要完成约 `n(n - 1) / 2` 次比较，因此时间复杂度始终为 O(n²)，额外空间 O(1)。一次跨区间交换可能越过相等元素，所以简单选择排序不稳定。

**易错点**：

- 保存的是最小元素的下标，不是在扫描过程中反复交换。
- 内层循环从 `i + 1` 开始，外层只需执行到 `n - 1`。
- 即使 `min == i`，当前源码的交换也不会改变结果。

**例题思路**：当题目强调“交换次数少”时，简单选择排序最多交换 `n - 1` 次，可以和冒泡排序对比。

### 7.4 直接插入排序 `InsertionSort`

**核心操作**：把 `a[i + 1]` 暂存在 `x` 中，从已有序区间 `1..i` 的末尾向前查找插入位置。比 `x` 大的元素依次右移，最后把 `x` 写入 `a[j + 1]`。

输入已有序时，每个元素只比较一次，时间复杂度 O(n)；逆序时需要大量移动，时间复杂度 O(n²)。只移动严格大于 `x` 的元素，因此相等元素相对顺序不变，算法稳定。

**易错点**：

- 插入前必须先保存 `x`，否则右移会覆盖待插入元素。
- 循环结束后插入位置是 `j + 1`，不是 `j`。
- 当前源码使用 1-based 下标，向前扫描的边界是 `j >= 1`。

**例题思路**：小规模或基本有序数据适合插入排序；若要求展示每一趟，记录每次将 `a[i + 1]` 插入后的数组即可。

### 7.5 希尔排序 `ShellSort`

**核心操作**：按增量 `d` 把相距 `d` 的元素分为若干组，在每组内做插入排序。当前实现使用折半增量 `d = n / 2, d /= 2`，最后一趟 `d = 1` 时完成整体有序。

源码中的移动步长是 `d`：比较 `a[j]` 与暂存值 `x`，较大元素移动到 `a[j + d]`。每完成一个增量，程序会输出“第 k 趟排序”和当前序列。

**复杂度与稳定性**：性能取决于增量序列，不能像普通插入排序一样只给出统一的紧确平均复杂度；当前折半增量的最坏情况通常按 O(n²) 复习。相等元素可能属于不同分组并跨越移动，所以不稳定。

**易错点**：

- 外层增量必须最终到达 `1`，否则不能保证整体有序。
- 分组插入时下标按 `d` 递减，而不是按 `1` 递减。
- 当前循环以 `d >= 1` 为条件；整数除法使 `d` 从 `1` 变为 `0` 后结束。

**例题思路**：手算时按增量逐趟分组，先写出每组下标，再在组内做直接插入，最后合并观察整张数组。

### 7.6 快速排序 `QuickSort`

**核心操作**：当前实现取区间首元素 `a[l]` 作为基准 `p`。左右指针从两端向中间移动，先从右侧找不大于基准的元素填到左侧空位，再从左侧找不小于基准的元素填到右侧空位；相遇后把 `p` 放入 `a[i]`，递归处理左右区间。

平均时间复杂度 O(n log n)，但已有序或逆序输入配合“首元素基准”容易产生极不均衡划分，最坏达到 O(n²)，递归深度也会达到 O(n)。交换和覆盖会改变相等元素的相对顺序，因此不稳定。

**易错点**：

- 当前“挖坑填数”写法必须先从右侧扫描，否则首元素基准的空位关系会被破坏。
- 两侧扫描都要带 `i < j`，避免指针越界或交错。
- 基准值要提前保存，指针相遇后再放回。
- 更稳健的实现可随机选择基准或使用三数取中，降低退化概率。

**例题思路**：求第 k 小元素时可借鉴一次划分后的基准位置，只递归目标所在的一侧，形成快速选择。

### 7.7 堆排序 `HeapSort`

**核心操作**：当前实现使用大根堆。先从 `n / 2` 到 `1` 调用 `downAdjust` 建堆；然后反复交换堆顶 `a[1]` 和当前堆尾，把最大值固定到数组右端，再缩小堆范围并向下调整。

`downAdjust(a, i, j)` 在区间 `1..j` 中比较节点和两个孩子，选择较大的孩子交换。源码还保留了 `upAdjust`，可用于逐个插入元素建堆，但当前 `main` 使用自底向上的 O(n) 建堆方式。

建堆 O(n)，随后 `n - 1` 次取堆顶和调整共 O(n log n)，最好、平均、最坏时间均为 O(n log n)，额外空间 O(1)。堆顶与末尾的远距离交换会改变相等元素次序，因此不稳定。

**易错点**：

- 1-based 堆中左孩子是 `2 * now`，右孩子是 `2 * now + 1`，父节点是 `now / 2`。
- 向下调整要先选出更大的孩子，再决定是否交换，才能维持大根堆。
- 每次交换堆顶后，已放到末尾的元素不再属于堆，调整上界应随轮次减小。
- 数组升序输出使用大根堆；若使用小根堆并同样交换到末尾，会得到降序结果。

**例题思路**：Top K 问题通常无需完整排序。求最大的 k 个元素可以维护容量为 k 的小根堆，使堆顶始终是当前候选中的最小值。

### 7.8 归并排序 `MergeSort`

**核心操作**：把区间 `[i, j]` 递归二分为两个子区间；当子区间长度为 1 时自然有序。回溯时用双指针比较左半区和右半区，把较小元素依次写入临时数组 `cnt`，再复制回原数组。

当前实现按 `1..n` 下标排序，比较使用 `a[k] <= a[q]`，相等时先取左半区元素，因此保持稳定性。每层合并处理 O(n) 个元素，共约 log n 层，时间复杂度稳定为 O(n log n)，额外空间 O(n)。

**Java 实现要点与易错点**：

- 递归终止条件是 `i >= j`；区间写错会导致无限递归或遗漏元素。
- 合并后两个剩余区间都要分别补入临时数组，不能只处理其中一边。
- 临时数组写回原数组时，当前实现从 `a[i]` 开始回填 `p - 1` 个元素；下标偏移错误是最常见问题。
- 当前每次合并都新建长度为 `105` 的 `cnt`，便于练习但会产生额外分配；工程实现通常复用一个与输入等长的辅助数组。

**例题思路**：归并过程可以顺带统计逆序对：当右侧元素先写入时，左半区尚未合并的元素都与它构成逆序对，数量为 `mid - k + 1`。

### 7.9 计数排序 `CountingSort`

**核心操作**：对每个非负整数 `x` 统计频数 `num[x]`，把频数转换成前缀和 `sum[x]`，使其表示不大于 `x` 的元素数量。随后从右向左扫描原数组，将 `x` 放到 `t[sum[x]]` 并递减 `sum[x]`，最后输出 `t[1..n]`。

设最大键值范围为 `k`，时间复杂度为 O(n + k)，空间复杂度为 O(n + k)。从右向左回填可保证相同键按原先相对顺序进入输出数组，所以当前实现稳定。

**Java 实现要点与易错点**：

- 当前 `num`、`sum` 的长度均为 `10005`，输入键必须在 `0..10004`；负数或过大数值会造成数组越界。
- 前缀和循环应覆盖 `1..max`，并先初始化 `sum[0] = num[0]`。
- 稳定版本不能只按计数直接重复输出；必须使用前缀和和输出数组，并从右向左扫描原数组。
- `num` 和 `sum` 是静态数组；同一 JVM 中若多次调用该逻辑，应先清零相关范围，避免上一次数据残留。

**例题思路**：当题目键值范围不大、整数且重复较多时，优先考虑计数排序。若范围远大于元素个数，`k` 会主导空间和时间，应改用比较排序或基数排序。
...
排序
├── 冒泡排序 ─────────────────── 相邻交换，已有序时可提前结束，稳定
├── 选择排序 ─────────────────── 每趟选择最小值，比较次数固定，不稳定
├── 插入排序 ─────────────────── 有序区间向后移动，适合基本有序数据，稳定
├── 希尔排序 ─────────────────── 分组插入，性能取决于增量序列，不稳定
├── 快速排序 ─────────────────── 基准划分+递归，平均 O(n log n)，不稳定
└── 堆排序 ───────────────────── 大根堆反复取最大值，O(n log n)，不稳定
```
